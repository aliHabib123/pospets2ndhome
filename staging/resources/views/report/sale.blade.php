                    ...
                    <table class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <?php if (Auth::user()->roles[0]->name == "Admin" || Auth::user()->roles[0]->name == "Manager") { ?>
                                    ...
                                    <td width="150">{{trans('report.rate')}}</td>
                                <?php } ?>
                                <td>&nbsp;</td>
                                <td>&nbsp;</td>
                            </tr>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td> {{$sales_currency}} {{ number_format(DB::table('sale_items')->where('sale_id', $value->id)->sum('total_selling'), 2)}}</td>
                                <td> {{$sales_currency}} {{ number_format($value->discount,2)}}</td>
                                <td> % {{ number_format($value->discount_percentage,2)}}</td>
                                <td style="font-weight: bold;"> {{$sales_currency}} {{ number_format(DB::table('sale_items')->where('sale_id', $value->id)->sum('total_selling') - $value->discount,2)}}</td>

                                <?php if (Auth::user()->roles[0]->name == "Admin" || Auth::user()->roles[0]->name == "Manager") { ?>
                                    <td> {{$sales_currency}} {{number_format(DB::table('sale_items')->where('sale_id', $value->id)->sum('total_cost'),2)}}</td>
                                    <td> {{$sales_currency}} {{number_format(DB::table('sale_items')->where('sale_id', $value->id)->sum('total_selling')
                                ...
                                <td colspan="10">
                                    <table class="table">
                                        ...
                                        <tr>
                                            ...
                                            <td>{{$sales_currency}} {{ number_format($SaleDetailed->selling_price ,2)}}</td>
                                            <td>{{$sales_currency}} {{ number_format($SaleDetailed->selling_price * $SaleDetailed->quantity,2)}}</td>
                                            <td>{{$sales_currency}} {{ number_format($SaleDetailed->discount ,2)}}</td>
                                        </tr>
                                        @endforeach
                                    </table>
                                </td>
