...
<section class="content-header">
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            {!! Form::open(array('url' => 'generalReports/wholesalessearch', 'method'=>'GET',)) !!}
            ...
            <ul class="list-inline">
                ...
                <?php if (Auth::user()->roles[0]->name == "Admin" || Auth::user()->roles[0]->name == "Manager" || Auth::user()->roles[0]->name == "Custom Role") { ?>
                    <li>
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td> {{$currency}} {{ number_format(DB::table('whole_sale_items')->where('sale_id', $value->id)->sum('total_selling'), 2)}}</td>
                                <td> {{$currency}} {{ number_format($value->discount,2)}}</td>
                                <td> % {{ number_format($value->discount_percentage,2)}}</td>
                                <td style="font-weight: bold;"> {{$currency}} {{ number_format(DB::table('whole_sale_items')->where('sale_id', $value->id)->sum('total_selling') - $value->discount,2)}}</td>

                                <?php if (Auth::user()->roles[0]->name == "Admin" || Auth::user()->roles[0]->name == "Manager") { ?>
                                    <td> {{$currency}} {{number_format(DB::table('whole_sale_items')->where('sale_id', $value->id)->sum('total_cost'),2)}}</td>
                                    <td> {{$currency}} {{number_format(DB::table('whole_sale_items')->where('sale_id', $value->id)->sum('total_selling')
                                     -DB::table('whole_sale_items')->where('sale_id', $value->id)->sum('total_cost')- $value->discount ,2)}}</td>
                                <?php } ?>
                                <td>
                                ...
                                <td> <a class="btn btn-small btn-warning" href="{{URL::to('/generalReports/refundwholesale')}}/{{ $value->id }}">Edit</a></td>
                                <td> <a class="btn btn-small btn-danger" href="{{URL::to('/generalReports/printWholesale')}}/{{ $value->id }}">Print</a></td>
                            </tr>
                            ...
                            <tr class="collapse" id="detailedSales{{ $value->id }}">
                                <td colspan="10">
                                    <table class="table">
                                        ...
                                        @foreach($value->wholeSaleItems as $SaleDetailed)
                                        <tr>
